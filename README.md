...
anbani
  ...
  │  ├─ convert [Function]
  │  ├─ interpret [Function]
  │  ├─ classify [Function]      // names the source script, or "unknown"
  │  └─ $                        // alias of classify; $.classifyText also works
  ├─ lorem
  │  ├─ sentences [Function]
  │  ├─ paragraphs [Function]
  ...
  ├─ ab
  │  ├─ letters { mkhedruli, asomtavruli, nuskhuri, mtavruli } [String]
  │  ├─ keys [Array]
  │  ├─ caps / bicam / bicaps [Function]
  └─ data
     ├─ alphabets   // per-script letter tables + names + numeric
     └─ regex       // per-script detection patterns
```
...
## 3.2
- New `core.classify` — the blessed name for script classification (matches the `core.classify` id in the cross-language golden spec).
